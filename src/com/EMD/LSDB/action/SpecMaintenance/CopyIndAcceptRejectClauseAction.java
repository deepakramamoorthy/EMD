 ...
 *
 */
public class CopyIndAcceptRejectClauseAction extends EMDAction {
	...
	public ActionForward fetchClauseVersions(ActionMapping objActionMapping,
		...
		LogUtil
		.logMessage("Entering CopyIndAcceptRejectClauseAction:fetchClauseVersions");
		...
		try {
			...
			//To Set  Order Selected Clause Version No in hidden field
			...
			//To Set  Copy From Verion Flag Indicator  Clause Version No in hidden field
			for (int j = 0; j < arlClauseList.size(); j++) {
				objClauseVO = (ClauseVO) arlClauseList.get(j);
				if (objClauseVO.getCopyFlag().equalsIgnoreCase("Y")) {
			...
			LogUtil
			.logMessage("Enters into Exception Block in CopyIndAcceptRejectClauseAction..");
			LogUtil
			.logMessage(" Exception occured in CopyIndAcceptRejectClauseAction:"
					+ objExp);
	...
	public ActionForward updateClauseChange(ActionMapping objActionMapping,
			...
			HttpServletResponse objHttpServletResponse) throws EMDException {
		LogUtil
		.logMessage("Enters into AcceptRejectClauseAction:UpdateAcceptClause Method");
		String strForwardKey = ApplicationConstants.SUCCESS;
		int intUpdate = 0;
		ArrayList arlClauseList = null;
		AcceptRejectClauseForm objAcceptRejectClauseForm = (AcceptRejectClauseForm) objActionForm;
		...
		try {
			...
			objAcceptRejectClauseForm.setRevCode(intrevCode);
			objAcceptRejectClauseForm.setSubSecSeqNo(intsubSecSeqNo);
			objAcceptRejectClauseForm.setIndFlag("Y");
...
//				Added for CR_114
			...
			objjClauseVO.setClauseSeqNo(intClauseSeqNo);
			objjClauseVO.setOrderKey(intOrderKey);
